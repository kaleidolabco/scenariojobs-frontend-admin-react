    ...
    return (
        <div className="bg-base-100 border border-base-200 rounded-xl p-4 space-y-2">
            <div className="flex items-center justify-between text-sm">
                ...
                <span className={`font-bold tabular-nums ${textCls}`}>{rounded}% / 100%</span>
            </div>
            ...
            <p className="text-xs text-base-content/50">
                {ok
                    ...
                    : over
                    ? `Excede en ${Math.round((rounded - 100) * 100) / 100}%. Reduce algunos pesos.`
                    : `Faltan ${Math.round((100 - rounded) * 100) / 100}% por asignar.`}
            </p>
        </div>
    );
};
